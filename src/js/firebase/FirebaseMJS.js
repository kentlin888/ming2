...
export default class FirebaseMJS {
     ...
     * @param {Boolean} value 
     */
    modifyOrderStatus(orderId, status, value) {
        this.initDB();
        let data = {
            orderStatus: {}
        }
        ...
        //data[status] = value
        let docRef = this._db.collection(FIRESTORE_COLLECTION.OrderInfo).doc(orderId);
    ...
    getOrderInfo = (userId, orderStatus) => {
        ...
        function getQuery(inOrderStatus) {
            switch (inOrderStatus) {
                ...
                case ENUM_orderStatus.waitForDelivery://待出貨
                ...
                case ENUM_orderStatus.waitForPay://已出貨,尚未到貨收款項
